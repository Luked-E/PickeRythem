    ...
    while(True):
        ...
        while(True):
            ...
            if(Timer2(nextPauseIn,lastPause)):
                ...
                print(chosenLight)
                nextRythemExponent = rythem[chosenLight]
                if (nextRythemExponent == rythem[chosenLight]):
...
gameTask = asyncio.create_task(Game())

#async input debugging
async def Debug():
    while(debugging):
        print(f"{in1.value}, {in2.value}, {in3.value}, {in4.value}")
        await asyncio.core.sleep(1)

    

async def Main():
    debugTask = asyncio.create_task(Debug())
    gameTask = asyncio.create_task(Game())

    await asyncio.gather(gameTask, debugTask)
    print("done")
